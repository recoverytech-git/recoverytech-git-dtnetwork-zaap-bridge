...
export async function POST(request) {
  ...
  try {
    ...
    if (!zaapUrl) {
      console.error(`[${requestId}] ZAAP_WEBHOOK_URL não configurada`);
    ...
    const payloadForZaap = {
      name,
      phone,
    };

    console.log(`[${requestId}] Payload recebido da DTNetwork`, {
      name,
      phone,
    });
    ...
    const zaapResponse = await fetch(zaapUrl, {
      ...
      body: JSON.stringify(payloadForZaap),
      cache: "no-store",
    });

    const zaapText = await zaapResponse.text();

    console.log(`[${requestId}] Resposta do Zaap`, {
      status: zaapResponse.status,
      ok: zaapResponse.ok,
    });
